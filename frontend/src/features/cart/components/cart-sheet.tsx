...
import { useGetCart, useClearCart } from "@/features/restaurants/api";
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      ₹{typeof item.itemId === "object" ? item.itemId?.price : 0}
                    </p>
                    <div className="flex items-center gap-2 mt-1.5">
                      <button
                        className="rounded-full border border-border p-0.5 text-muted-foreground hover:bg-accent transition-colors"
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <p className="text-sm font-semibold">
                      ₹{(typeof item.itemId === "object" && item.itemId?.price ? item.itemId.price : 0) * (item.quantity || 1)}
                    </p>
                    <button
                      className="text-xs text-destructive hover:text-destructive/80 transition-colors inline-flex items-center gap-0.5"
          ...
          <div className="border-t border-border/50 px-4 py-4 space-y-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="inline-flex h-10 items-center justify-center rounded-xl border border-border px-4 text-sm font-medium text-muted-foreground hover:bg-accent transition-colors gap-1.5"
              >
                <Trash2 className="h-4 w-4" />
                Clear
              </button>
              <button className="flex-1 inline-flex h-10 items-center justify-center rounded-xl bg-primary px-5 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors gap-1.5 shadow-sm">
                Checkout
                <ArrowRight className="h-4 w-4" />
              </button>
            </div>
